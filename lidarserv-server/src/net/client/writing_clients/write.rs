...
pub struct WriteClient {
    pub(super) connection: Connection<TcpStream>,
    pub(super) coordinate_system: CoordinateSystem,
    pub(super) attributes: Vec<PointAttributeDefinition>,
    pub(super) codec: PointDataCodec,
}
#[async_trait]
pub trait Connect: Sized {
    async fn connect<A>(addr: A, shutdown: &mut Receiver<()>) -> Result<Self, LidarServerError> where A: ToSocketAddrs + Send;
    
}

pub trait InformationProvider{
    fn coordinate_system(&self) -> CoordinateSystem;

    fn attributes(&self) -> &[PointAttributeDefinition];

    fn codec(&self) -> PointDataCodec;

}

#[async_trait]
impl Connect for WriteClient {
    async fn connect<A>(addr: A, shutdown: &mut Receiver<()>) -> Result<Self, LidarServerError>
    where
        ...
        connection
            .write_message(
                &Header::ConnectionMode {
                    device: DeviceType::UpdateClient,
                },
                &[],
            )
            .await?;
...
impl InformationProvider for WriteClient { 
    fn coordinate_system(&self) -> CoordinateSystem {
